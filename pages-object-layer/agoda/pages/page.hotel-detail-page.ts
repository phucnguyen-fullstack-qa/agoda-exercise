...
export class AgodaHotelDetailPage extends AgodaBasePage {
  ...
  get elements() {
    return {
      ...
      bookButton: (roomIndex: number, offerIndex: number) =>
        this.elements.roomOfferByIndex(roomIndex, offerIndex)
          .locator('[data-element-name="mob-room-tile-book-now"]')
          .first(),
    };
  }
  ...
  async selectRoomAndBook(
    ...
    const price = (
      await offer.getByTestId('room-offer-final-price').innerText()
    ).trim();
