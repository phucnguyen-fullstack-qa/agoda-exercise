...
export class AgodaSearchResultPage extends AgodaBasePage {
    get elements() {
        return {
            ...
            hotelNameByText: (hotelName: string) => this.elements.hotelItems().locator('[data-selenium="hotel-name"]').filter({ hasText: hotelName }).first(),
            price: (hotel: Locator) => hotel.locator('[data-element-name="final-price"] [data-selenium="display-price"]'),
    ...
    async selectAvailableHotelWithSuggestedPrice(expectedHotelName?: string): Promise<SelectedHotel> {
        ...
        for (const hotel of hotels) {
            ...
            const newPage = await this.performActionAndWaitForNewPage(() => hotel.click());
